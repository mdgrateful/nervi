...
export function isSubscriptionActive(subscriptionStatus, hasLifetimeAccess = false) {
  // Users with promo codes have lifetime access
...
export function hasPremiumAccess(subscriptionTier, subscriptionStatus, hasLifetimeAccess = false) {
  // Users with promo codes get premium access
  if (hasLifetimeAccess) {
    return true;
  }
  return (
    subscriptionTier === 'premium' &&
    isSubscriptionActive(subscriptionStatus, hasLifetimeAccess)
  );
}

// Helper function to check if user has any valid access (subscription or promo code)
export function hasValidAccess(user) {
  if (!user) return false;

  // Check for lifetime access (promo code)
  if (user.has_lifetime_access || user.hasLifetimeAccess) {
    return true;
  }

  // Check for active subscription or trial
  const subscriptionStatus = user.subscription_status || user.subscriptionStatus;
  return isSubscriptionActive(subscriptionStatus, false);
}
