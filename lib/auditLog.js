...
import { sanitizePII } from "./logger";
...
export const auditLog = {
  ...
  loginFailed: (identifier, reason, request) => {
    logAuditEvent(
      "auth.login.failed",
      null,
      { action: "Login attempt failed", identifier: sanitizePII(identifier), reason },
      request
    );
  },
  ...
  accountCreated: (userId, username, request) => {
    logAuditEvent(
      "account.created",
      userId,
      { action: "New account created", username: sanitizePII(username) },
      request
    );
  },
  ...
  accountDeleted: (userId, username, request) => {
    logAuditEvent(
      "account.deleted",
      userId,
      { action: "Account permanently deleted", username: sanitizePII(username) },
      request
    );
  },
  ...
  promoCodeApplied: (userId, code, request) => {
    logAuditEvent(
      "promo.applied",
      userId,
      { action: "Promo code applied", code: sanitizePII(code) },
      request
    );
  },
  ...
  rateLimitExceeded: (identifier, endpoint, request) => {
    logAuditEvent(
      ...
      null,
      { action: "Rate limit exceeded", identifier: sanitizePII(identifier), endpoint },
      request
    );
  },
  ...
  adminAction: (adminUserId, action, targetUserId, request) => {
    logAuditEvent(
      "admin.action",
      adminUserId,
      { action: "Admin action performed", adminAction: action, targetUserId: sanitizePII(targetUserId) },
      request
    );
  },
